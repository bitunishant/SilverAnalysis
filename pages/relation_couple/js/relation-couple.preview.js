...
export function applyToPreview() {
  ...
  if (p1Age)
    p1Age.textContent = document.getElementById("person1Age")?.value || "";
  if (p2Age)
    p2Age.textContent = document.getElementById("person2Age")?.value || "";
  if (rDate)
    rDate.textContent = document.getElementById("reportDate")?.value || "";
  ...
  ["person1", "person2"].forEach((person) => {
    ["signature", "handwriting"].forEach((type) => {
      const fileInput = document.getElementById(
        `${person}${type.charAt(0).toUpperCase() + type.slice(1)}`,
      );
